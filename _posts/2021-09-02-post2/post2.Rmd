...
prefectures <- prefectures_data %>%
  ...
  top_n(6) 
   ...
   table<-prefectures %>% rename( Prefectures = prefectureNameE,
          "Number of positive cases" = postive_cases)
...
p_plot <- ggplot(data = prefectures, 
   ...
   geom_text(aes(label = postive_cases ), vjust =2, colour = "black", size = 2.2) +
