...
table2 <- Philippine %>% 
  ...
  mutate(mortality_rate = sum(total_deaths/population),
         fatality_rate = sum(total_deaths/total_cases),
         avg_number_of_cases_daily = sum(total_cases/730),
        avg_number_of_deaths_daily = sum(total_cases/730)) %>% 
        select(-c("new_cases",
                  "new_deaths",
                  "location"))
...
v_plot1 <- combine %>% 
  ...
  geom_histogram(alpha = 0.2, position = "dodge", stat = "identity" )
...
table1 <- combine %>% 
  ...
  group_by(date,Type) %>% 
  summarise(percentage = sum(number/111,046,910)) 
...
```{r}
association <- 
  ggplot( data = Philippine_v ,aes(x = people_fully_vaccinated,
          y = total_deaths,
          colour = total_cases) )  +
  geom_point(size = 0.3) +  
    ggtitle("Association between the number of deaths and vaccination") +
  xlab("Total number of people fully vaccinated") + 
  ylab("Total deaths ")

a <- ggplotly(association)
a
association
```
